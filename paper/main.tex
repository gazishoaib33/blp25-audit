...
more than 3 of the 29 sexist comments, and five detect none. We release all
audit code.
\end{abstract}
...
\paragraph{Significance in NLP.} \citet{reimers2017reporting} showed single-run
comparisons are unreliable, \citet{dror2018hitchhiker} surveyed appropriate
tests, and \citet{card2020power} showed many NLP experiments are underpowered for
the effects they claim. We apply this lens to a shared-task leaderboard rather
than to one paper's ablations.
...
text-to-SQL. Closest to our \S\ref{sec:metric},
\citet{harbecke2022micro} show that rankings shift with the weighting scheme and
argue against reporting micro-F1 alone; we instead bound what micro-F1 can
register on a given benchmark. \citet{vidgen2020directions} survey quality problems specific to
...
discordance, so we measure it. We trained ten systems on the official train split
and evaluated each once on the test split, varying feature space, classifier
family, regularisation and class weighting. Micro-F1 ranges 0.4984--0.6776; these
are classical models and do not reach the 0.7362 of the shared task's transformer
ensembles (see Limitations).
...
hold their order in 98.7\% of 4{,}000 resamples. The claim is conditional---a gap
is uninformative when smaller than $1.96\sqrt{b+c}/n$---so what condemns the BLP
gaps is their size \emph{relative to plausible discordance}. In
Figure~\ref{fig:gapwin}, the one pair inside the BLP range wins 51.5\% of
resamples; every pair we could resolve had a gap of at least 0.44\,pp.
...
the best detects 3 (F1 0.115). That same system has the best macro-F1, 0.5019,
and ranks only \emph{fourth} of ten on micro-F1; micro and macro rankings agree
weakly (Kendall $\tau=0.467$, $p=.073$). The reported metric does not merely fail
to reward rare-class detection---in our pool it demotes the system that does it
best.
...
Five changes, all cheap. \textbf{Report confidence intervals} --- one line of
code, and it would have shown these five-way ties immediately. \textbf{Release
per-system predictions,} without which McNemar is uncomputable outside the
organising committee. \textbf{Report macro-F1 alongside micro-F1} on skewed
distributions, so rare classes stay visible. \textbf{Validate schema coherence at
release}; Table~\ref{tab:coherence} is mechanical. And \textbf{declare ties} ---
ordering noise manufactures a false narrative of progress.

The dataset is well constructed; the evaluation cannot support the rankings it
reports. The top five are tied in all three subtasks, the plateau is not
annotation noise, and the metric is blind to the rarest category.
